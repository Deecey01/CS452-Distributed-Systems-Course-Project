...
// Batch represents a batched set of commands to improve throughput
type Batch struct {
	Commands []interface{} // Commands is the slice of batched commands
}

func init() {
	// Ensure Batch is registered with gob so it can be encoded/decoded
	// when sent as a LogEntry.Command or inside RPC payloads. Tests and
	// the main program may run in different packages/processes, but
	// registering here guarantees the type is known when the raft
	// package is initialized in tests.
	gob.Register(Batch{})
}
...
type RaftNode struct {
	...
	// Batching state (only used by leader)
	batchBuffer      []interface{}  // batchBuffer holds commands waiting to be batched
	batchMu          sync.Mutex     // batchMu protects batchBuffer
	maxBatchSize     int            // maxBatchSize is the maximum number of commands in a batch
	maxBatchWait     time.Duration  // maxBatchWait is the maximum time to wait before flushing a batch
...
func (rn *RaftNode) sendCommit() {
	for range rn.newCommitReady {
		...
		for i, entry := range entries { // For each entry in the slice of log entries that we have not applied yet
			// Debug: Log if we're sending a Batch to commit channel
			if batch, isBatch := entry.Command.(Batch); isBatch {
				rn.debug("sendCommit: Sending BATCH with %d commands to commitChan", len(batch.Commands))
			}
			rn.commitChan <- CommitEntry{ // Send the entry to the commit channel
				...
				Index:   savedLastApplied + uint64(i) + 1, // Index is the log entry
...
func (rn *RaftNode) becomeFollower(term uint64) {
	...
	// Clear any pending batch since we're no longer leader
	rn.batchMu.Lock()
	if rn.batchTimerActive && rn.batchTimer != nil {
		rn.batchTimer.Stop()
		rn.batchTimerActive = false
	}
	rn.batchBuffer = rn.batchBuffer[:0] // Clear buffer without deallocating
	rn.batchMu.Unlock()
...
func (rn *RaftNode) flushBatch() {
	...
	rn.batchBuffer = rn.batchBuffer[:0]

	// Trigger replication (must be done after releasing mu to avoid deadlock)
	// We'll do this in the caller after they release mu
}

// tryAddToBatch attempts to add a command to the batch buffer.
// Returns true if command was batched, false if it should be handled directly.
// This function acquires batchMu and mu locks internally.
func (rn *RaftNode) tryAddToBatch(command interface{}) bool {
	// Only batch Write commands (not Read, AddServers, RemoveServers)
	switch command.(type) {
	case Write:
		// This is batchable
	default:
		return false // Not batchable
	}

	rn.batchMu.Lock()
	defer rn.batchMu.Unlock()

	// Check if we're still the leader (double-check pattern)
	rn.mu.Lock()
	...
	// Add to batch buffer
	rn.batchBuffer = append(rn.batchBuffer, command)
	bufferSize := len(rn.batchBuffer)

	// Start timer if not already active
	if !rn.batchTimerActive {
		rn.batchTimer = time.AfterFunc(rn.maxBatchWait, func() {
			rn.batchMu.Lock()
			defer rn.batchMu.Unlock()
			...
			// Trigger replication after releasing locks
			if needsTrigger {
				select {
				case rn.trigger <- struct{}{}:
				default:
					// Trigger channel is buffered, skip if full
				}
			}
		})
		rn.batchTimerActive = true
	}

	// Flush if batch is full
	if bufferSize >= rn.maxBatchSize {
		...
		if rn.state == Leader {
			rn.flushBatch()
			rn.mu.Unlock()
			// Trigger replication after releasing lock
			select {
			case rn.trigger <- struct{}{}:
			default:
				// Trigger channel is buffered, skip if full
...
func (rn *RaftNode) Submit(command interface{}) (bool, interface{}, error) {
	...
	if rn.state == Leader {
		...
		case Write:
			// Unlock mu before trying to batch (tryAddToBatch handles locks)
			rn.mu.Unlock()
			
			// Try to add to batch buffer
			if rn.tryAddToBatch(command) {
				// Successfully batched
				return true, nil, nil
			}
			
			// If batching failed, fall back to immediate append
			// (shouldn't happen for Write, but handle gracefully)
			rn.mu.Lock()
			if rn.state != Leader {
				rn.mu.Unlock()
		...
		case RemoveServers:
			...
			return true, nil, nil      // Return true since we are the leader
		default:
			// For unknown command types, append directly without batching
			rn.log = append(rn.log, LogEntry{Command: command, Term: rn.currentTerm}) // Append the command to the log
			rn.persistToStorage()                                                     // Persist the log to storage
			rn.debug("log=%v", rn.log)                                                // Debug the log state
			rn.mu.Unlock()                                                            // Unlock the mutex before returning
