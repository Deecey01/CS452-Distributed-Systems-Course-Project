...
const PreVoteRPCTimeout = 150 * time.Millisecond
...
func (rn *RaftNode) runElectionTimer() {
	...
	for {
		...
		// If the term has changed, this timer is stale
		...
		// or haven't voted for someone for the duration of the timeout.
		if elapsed := time.Since(rn.electionResetEvent); elapsed >= timeoutDuration {
			// IMPORTANT: release lock *before* calling startElection.
			// startElection will spawn RPC goroutines that need to acquire rn.mu.
			// Calling startElection while holding rn.mu causes deadlock.
			rn.mu.Unlock()
			// Call startElection asynchronously so it can run on its own
			// and the timer goroutine can return.
			go rn.startElection()
			return
		}
...
// startElection starts a new election. It first runs a PreVote
// round (IsPreVote=true, Term = currentTerm+1). Only if PreVote
// gets a majority do we increment currentTerm and run a real vote.
func (rn *RaftNode) startElection() {
	// Snapshot values under lock to use while sending RPCs.
	savedLastLogIndex, savedLastLogTerm := rn.lastLogIndexAndTerm()
	savedCurrentTerm := rn.currentTerm

	rn.debug("Election timeout -> starting PreVote (term=%d, lastLog=(%d,%d))", savedCurrentTerm, savedLastLogIndex, savedLastLogTerm)
	...
	// prospective term candidate would use if it started a real election
	prospectiveTerm := savedCurrentTerm + 1
	...
	for peer := range rn.peerList.peerSet {
		...
		go func(peer uint64) {
			...
			rn.debug("Sending PreVote(RequestVote IsPreVote=true) to %d: %+v", peer, args)

			// run RPC with per-call timeout to avoid indefinite blocking
			replyCh := make(chan RequestVoteReply, 1)
			errCh := make(chan error, 1)
			go func() {
				var reply RequestVoteReply
				err := rn.server.RPC(peer, "RaftNode.RequestVote", args, &reply)
				if err != nil {
					errCh <- err
					return
				}
			...
			case <-time.After(PreVoteRPCTimeout):
				rn.debug("PreVote RPC to %d timed out", peer)
				return
			case err := <-errCh:
				rn.debug("PreVote RPC to %d returned error: %v", peer, err)
				return
			case reply := <-replyCh:
				// If the peer reports a higher term, step down immediately.
				rn.mu.Lock()
				if reply.Term > rn.currentTerm {
					rn.debug("PreVote discovered higher term %d from %d -> stepping down", reply.Term, peer)
	...
	// wait for pre-vote replies (bounded by PreVoteRPCTimeout)
	wg.Wait()

	totalNodes := rn.peerList.Size() + 1
	rn.debug("PreVote results: got %d/%d (need >%d)", preVotes, totalNodes, totalNodes/2)

	// If no majority, do not increment term or change votedFor; reset election timer and return.
	if preVotes*2 <= totalNodes {
		rn.debug("PreVote failed (%d/%d) - not starting real election", preVotes, totalNodes)
		rn.mu.Lock()
	...
	// ---------- PreVote succeeded -> perform actual election ----------
	rn.mu.Lock()
	// double-check state hasn't changed
	if rn.state == Dead || rn.state == Leader {
		rn.mu.Unlock()
		return
	}
	rn.state = Candidate
	rn.currentTerm = prospectiveTerm
	savedCurrentTerm = rn.currentTerm
	rn.electionResetEvent = time.Now()
	...
	rn.debug("PreVote succeeded; Becomes Candidate (currentTerm=%d); log=%v", savedCurrentTerm, rn.log)
	...
	go func() {
		...
		if votesReceived*2 > totalNodes {
			rn.debug("Wins election with %d votes (single-node fast path)", votesReceived)
			rn.becomeLeader()
		}
	}()

	// Send real RequestVote RPCs with IsPreVote=false (bounded by timeout)
	...
	for peer := range rn.peerList.peerSet {
		...
		go func(peer uint64) {
			...
			rn.mu.Lock()
			lastIdx, lastTerm := rn.lastLogIndexAndTerm()
			rn.mu.Unlock()
			...
			select {
			case <-time.After(PreVoteRPCTimeout):
				rn.debug("RequestVote RPC to %d timed out", peer)
				return
			case err := <-errCh:
				rn.debug("RequestVote RPC to %d returned error: %v", peer, err)
				return
			case reply := <-replyCh:
				rn.mu.Lock()
				...
				if rn.state == Candidate && reply.Term == savedCurrentTerm && reply.VoteGranted {
					votesMu.Lock()
					votesReceived++
					votesMu.Unlock()
					if votesReceived*2 > totalNodes {
	...
	// wait bounded by PreVoteRPCTimeout
	wg2.Wait()

	// If election didn't win, restart election timer
	rn.mu.Lock()
	if rn.state != Leader {
		rn.electionResetEvent = time.Now()
	}
...
func (rn *RaftNode) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	rn.debug("RequestVote: %+v [currentTerm=%d, votedFor=%d, log index/term=(%d, %d)]",
		args, rn.currentTerm, rn.votedFor, lastLogIndex, lastLogTerm)

	// ---- PreVote handling: do not modify persistent state ----
	if args.IsPreVote {
		// If the prospective term is older than our current term, deny.
		if args.Term < rn.currentTerm {
			reply.VoteGranted = false
			reply.Term = rn.currentTerm
			rn.debug("RequestVote (pre-vote) reply: %+v", reply)
			return nil
		}
		// Otherwise grant if candidate's log is at least as up-to-date as ours.
		if args.LastLogTerm > lastLogTerm || (args.LastLogTerm == lastLogTerm && args.LastLogIndex >= lastLogIndex) {
			reply.VoteGranted = true
		} else {
			reply.VoteGranted = false
		}
		// Return our current term (do not update it).
		reply.Term = rn.currentTerm
		rn.debug("RequestVote (pre-vote) reply: %+v", reply)
		return nil
	}

	// ---- Normal RequestVote handling ----

	// If the candidate has a higher term, become follower first.
	if args.Term > rn.currentTerm {
